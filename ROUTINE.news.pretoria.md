# PretoriaHub daily news routine — runbook
...
PretoriaHub has a Local news page (`/news/`) and a "today" panel on the
homepage. Your job is to find the day's genuinely useful local news for
the City of Tshwane metro (Pretoria, Centurion and surrounds) — the things that change someone's day: **traffic and road
...
file to `db/routine-updates/pretoria/` — the same directory and GitHub Actions
apply-on-deploy mechanism the other routines use. Your job ends at "commit and
push the SQL file".

## Reading current state

`status/pretoria/db-snapshot.json`'s `news` array lists every article already
...
the City of Tshwane metro (Pretoria, Centurion and surrounds): City of Tshwane media releases, SANRAL, Gautrain, Pretoria News, Rekord, Netwerk24/Maroela Media, News24, IOL, EWN, Tshwane Tourism, Loftus Versfeld / Blue Bulls / Mamelodi Sundowns. Official bodies (municipality, SANRAL, provincial
...
```
node scripts/check-news.mjs db/routine-updates/pretoria/<your file>.sql --city pretoria --online
```
...
Append one line to `status/pretoria/news-agent-log.jsonl` every run:
