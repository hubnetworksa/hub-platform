...
Status as of 21 September 2026. **Phases 2 and 3 are built on the dev branch** (see `routines/README.md`); nothing is scheduled or on `main` yet. Phase 0 (batched deploys, lighter build reads) and the new script-only routines (backup, health report, tidy-up) are still to build. Scope: TheCapeTownHub, PretoriaHub and PolokwaneHub. **[Me]** = I build it. **[You]** = needs your accounts or a decision.
