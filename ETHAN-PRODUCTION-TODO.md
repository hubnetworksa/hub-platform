...
|---|---|---|---|
| 2.1 | Run the rest of the payment flow on the PayFast sandbox: payment notification (ITN), cancel, expiry, sponsorship purchase, rejected-listing refund note. Checkout itself is verified (see done list): a real tier-upgrade checkout was signed, sent to the PayFast sandbox and **accepted** (redirected to a real payment page) from the live Ethan preview. | Me | Preview now has its own PayFast sandbox credentials (see done list), no longer blocked on you |
