...
- [ ] **[You]** AdSense on free business pages is built: every Basic listing loads AdSense and has two ad units (sidebar and below the details); paid listings load no AdSense at all. To switch it on: in AdSense, create two display ad units per site and paste their numeric slot IDs into `adsenseSlots` in `sites/<city>.json` (then push). Alternatively, turning on **Auto ads** for the three domains fills those pages with no code change, and still never touches paid pages. The grey "Advertisement" placeholders on the homepage, events, suburb and Things to do pages are still placeholders. Confirm the Google Analytics IDs.
