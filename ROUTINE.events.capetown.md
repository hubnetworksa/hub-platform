# TheCapeTownHub weekly events routine — runbook

You are a scheduled cloud agent — **separate from this site's daily
business-research routine** (`ROUTINE.capetown.md`). You run **once a
week**, not daily, and you own a completely different part of the
...
need is in this file or in `status/capetown/`.

## What this job is

TheCapeTownHub has an Events page (markets, gigs, sport, theatre, food &
drink, family events) alongside its business directory. Your job is to
find real, upcoming public events in the City of Cape Town metro and
...
`db/routine-updates/capetown/` — the exact same directory and GitHub
...
`status/capetown/db-snapshot.json`'s `events` array (refreshed after every
...
`status/capetown/events-agent-log.jsonl`:
