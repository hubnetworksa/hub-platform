...
routine (`ROUTINE.events.capetown.md`). You run **once a day** and own two
...
prices for this site's region — **coastal** for TheCapeTownHub. You keep them current:

1. Check `status/capetown/db-snapshot.json`'s `fuel_prices` array. If the current
   month (`YYYY-MM`) already has the four `coastal` grades, do nothing here.
2. Otherwise, once the new prices are announced, find the official
   ...
   a major news outlet). Read them. Take the **coastal** prices per litre for:
   ...
   coastal region column — Gauteng/Limpopo are inland, Cape Town is coastal.)
