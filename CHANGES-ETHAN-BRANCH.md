...
- **Two more real PayFast bugs found and fixed in `functions/_lib/payfast.ts`, both in Cancel.** The Subscriptions REST API signs requests differently from checkout (passphrase sorted in alphabetically, not appended last) — getting it wrong silently looked like a credentials problem ("Merchant authorization failed"). Also, `cancelPayfastSubscription` now checks the response body, not just the HTTP status: PayFast can return HTTP 200 with a body saying the action failed, which would otherwise have told an owner their subscription was cancelled while PayFast kept charging them.
- **Verified live against the real database and the real PayFast sandbox:** tier checkout, sponsorship checkout, the sponsorship exclusivity guard, the subscription expiry sweep (downgrade, slot freed, re-purchasable), Cancel's failure path, and the rejected-paid-listing refund note.
- **Preview environment gained a fresh `CRON_SECRET`** (an internally-invented value, not a third-party secret) via the same `env.preview` mechanism, so the admin sweep endpoints are testable on the dev preview.
- **Events now get a free, non-copyrighted, AI-generated image when no real photo was found.** New `scripts/generate-event-images.mjs` uses Cloudflare Workers AI (`@cf/black-forest-labs/flux-1-schnell`, no external API key) to generate a themed flat-illustration image matching the event's type, stores it in R2, and links it — never overwriting a real photo. `routines/events.md` updated to call it as the fallback step after the agent's own image search comes up empty. Run live for all 20 imageless PretoriaHub events on 21 September.
