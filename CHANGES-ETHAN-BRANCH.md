...
- **Fuel prices split from the news routine:** new `ROUTINE.fuel.md` (one monthly routine for all three cities) and `scripts/fuel-due.mjs` (only due on the first Wednesday and the two days after). Fuel sections removed from the three `ROUTINE.news.<city>.md` runbooks.
