...
PolokwaneHub is a business directory for Polokwane, Seshego and the greater
Capricorn District of Limpopo. Structured data (businesses, suburbs, categories) lives in a
...
- `status/polokwane/db-snapshot.json` (committed to this repo) is a snapshot of
...
- You propose changes by writing a SQL file to `db/routine-updates/polokwane/`. A
...
`status/polokwane/db-snapshot.json`, leave its row alone entirely, even if you notice
...
Read `status/polokwane/routine-state.json`:

```json
{ "suburb_index": 0, "suburbs_per_run": 3, "suburb_slugs": ["polokwane-central", "bendor", ...] }
```
...
- **Both a phone number AND a street address are mandatory to publish**
  ...
  so a phone number for "the Steers in Polokwane" is worthless if there
...
- **Never name an individual in a description** (per owner decision
  2026-09-04). Don't write the owner, founder, director, partner, or an
  individual practitioner's personal name into `description` — not even
  when a source states it plainly, and not for doctors/dentists/attorneys
  whose names are on public professional registers. Describe what the
  business *does*, not who runs it: "an owner-managed gym", "a general
  practice", "founded in 2013", "a family-run butchery" are all fine;
  "run by Jane Doe", "founded by Dr John Smith" are not. The business's
  own registered name is not covered by this even when it contains a
  person's name (e.g. a practice literally called "Dr VH Meintjes") — use
  the name as it appears in the `name` field, just don't expand initials
  to a full name or add first names it doesn't already show.
...
- **Tenant discovery, new or existing centre — go through the whole
  ...
  against `status/polokwane/db-snapshot.json`'s businesses already carrying that
...
because its coordinates land nearby produces real, shipped bugs — a business
...
2. **Fetch its tenant/store list.** Most real malls have a "Stores",
   ...
   *complete* list with confidence, don't guess at partial coverage —
   ...
   in a snippet.
3. **Cross-reference against `status/polokwane/db-snapshot.json`:**
...
1. Actively search for shopping centres/malls in Polokwane and the greater
   ...
   Polokwane`, or target a suburb you know is dense with retail
...
2. For each new one you find, verified with the same 2-independent-source
   ...
   `status/polokwane/db-snapshot.json`. **The existing-business-linking case (step
...
Any businesses added by a bulk import (`origin = 'seed_import'`) tend to
carry a generic, copy-pasted description: *"`<Name>` is a business in
`<Suburb>`, part of the Greater Polokwane area."* — identical in
shape for every one, saying nothing about what the business
actually does. This job rewrites them, one real business at a time, with
something a human actually researched and wrote. If the dataset currently
has no `seed_import` backlog (every row is `agent_research`, each already
individually described), job 4 simply has nothing to do this run — note
that and move straight to job 3.

**Finding this run's batch:** in `status/polokwane/db-snapshot.json`'s `businesses`
...
1. **Research it.** Search `"<name>" <suburb> Polokwane` and, if the
...
`db/routine-updates/polokwane/<UTC timestamp, e.g. 2026-08-31T10-00-00>.sql`. Every
...
see "Committing") to `status/polokwane/agent-log.jsonl` (create if missing), as part
of that checkpoint's commit:

```json
{"date": "2026-08-31T10:00:00Z", "suburb": "bendor", "action_taken": "research", "published_new": 3, "discarded_unverified": 4, "shopping_centers_added": 1, "short_summary": "Bendor: added 3 new businesses (all with verified phone numbers), added Example Mall with 2 verified tenants."}
```
...
Each commit: write a new `db/routine-updates/polokwane/<UTC timestamp, e.g.
