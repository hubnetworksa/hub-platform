...
// email (see functions/api/claim-event.ts). Mirrors functions/review-claim.ts.
export const onRequestGet: PagesFunction<Env> = async (context) => {
  const site = getSite(context.env.SITE);
  const token = new URL(context.request.url).searchParams.get('token') ?? '';
  const row = await context.env.DB
    .prepare('SELECT ec.id, ec.contact_name, ec.contact_phone, ec.contact_email, ec.role_note, ec.status, e.title AS event_title, u.email AS claimant_email FROM event_claims ec JOIN events e ON e.id = ec.event_id JOIN users u ON u.id = ec.user_id WHERE ec.review_token = ?')
    .bind(token)
    .first<ClaimRow>();
