...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  const owned = await db
    .prepare('SELECT id, slug, name, subscription_tier, subscription_status FROM businesses WHERE owner_user_id = ? ORDER BY name')
    .bind(user.id)
    .all<{ id: number; slug: string; name: string; subscription_tier: number; subscription_status: string | null }>();
  ...
  return json({
    ok: true,
    email: user.email,
    owned: owned.results.map((b) => ({
      id: b.id,
      slug: b.slug,
      name: b.name,
      subscriptionTier: b.subscription_status === 'active' ? b.subscription_tier : 0,
    })),
