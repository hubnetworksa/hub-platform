# PolokwaneHub weekly events routine — runbook

You are a scheduled cloud agent — **separate from this site's hourly
business-research routine** (`ROUTINE.polokwane.md`). You run **once a
...
need is in this file or in `status/polokwane/`.

## What this job is

PolokwaneHub has an Events page (markets, gigs, sport, theatre, food &
drink, family events) alongside its business directory. Your job is to
find real, upcoming public events in the Polokwane metro and
...
`db/routine-updates/polokwane/` — the exact same directory and GitHub
...
`status/polokwane/db-snapshot.json`'s `events` array (refreshed after every
...
`status/polokwane/events-agent-log.jsonl`:
